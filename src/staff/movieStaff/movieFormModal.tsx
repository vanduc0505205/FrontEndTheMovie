...
export default function MovieModal({
  ...
  const handleUpload = async (file: RcFile, type: "poster" | "banner") => {
    try {
      ...
      const fileObj = { uid: file.uid, name: file.name, status: "done" as const, url };

      if (type === "poster") {
        setPosterUrl(url);
        setPosterFileList([fileObj]);
        form.setFieldsValue({ poster: url });
      } else {
        setBannerUrl(url);
        setBannerFileList([fileObj]);
  ...
  const handleFinish = async (values: any) => {
    try {
      ...
      if (!isEditing) {
        ...
        if (exists.some((movie: IMovie) => movie.title === formatted.title)) {
          return Modal.error({
      ...
      <Form form={form} layout="vertical" onFinish={handleFinish}>
        <Row gutter={16}>
          <Col span={12}>
            ...
            <Form.Item name="categories" label="Danh mục" rules={[{ required: true, message: "Vui lòng chọn ít nhất một danh mục" }]}>
